...
function init(_userSettings,httpServer,_adminApi) {
	...
	// Agregar el nuevo endpoint para obtener el token JWT y el rol
	adminApp.get('/jwtoken', function(req, res) {
		try {
			...
			const userRole = decodedToken.role || 'Role not found in token';

			res.json({ jwtToken: jwtToken, role: userRole });
		} catch (error) {
