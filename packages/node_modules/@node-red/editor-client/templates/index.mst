...
async function getJwtToken() {
    return jwtToken = eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InByb0B1bWEuZXN3IiwiaWQiOiI2NjU4ZjE5NDkwZGNiNDFmMzQ0YjhmODQiLCJpYXQiOjE3MTgxMzk5MzksImV4cCI6MTcxODE0MzUzOX0.n5-bNzDrldXcdYWa7NnSMKFRJAjxoZFdylSEBWllvgE;
}
...
async function showFlowListLoad() {
    try {
  jwtToken = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InByb0B1bWEuZXN3IiwiaWQiOiI2NjU4ZjE5NDkwZGNiNDFmMzQ0YjhmODQiLCJpYXQiOjE3MTgyMjgyMTMsImV4cCI6MTcxODIzMTgxM30.4bjrxfGHbjJvllGdcwThfy2l2YwU2kpioAjQjREMN4Q";
            ...
            listItem.onclick = async function() {
                try {
                    ...
                    const response = await fetch(`http://127.0.0.1:1880/flow/`, {
                     ...
                     console.log("aqui"+ JSON.parse(flow.nodes).shift())
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
                    }
                } catch (error) {
...
// Función para crear un nuevo flujo en la base de datos
async function createNewFlow(datos) {
    const jwtToken = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InByb0B1bWEuZXN3IiwiaWQiOiI2NjU4ZjE5NDkwZGNiNDFmMzQ0YjhmODQiLCJpYXQiOjE3MTgyMjgyMTMsImV4cCI6MTcxODIzMTgxM30.4bjrxfGHbjJvllGdcwThfy2l2YwU2kpioAjQjREMN4Q";
    try {

        console.log('Datos a enviar:', datos); // Añadir depuración
        const response = await fetch('http://localhost:3000/api/v1/flow/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        console.log('Response status:', response.status); // Añadir depuración
        if (response.ok) {
            const responseData = await response.json();
            console.log('Response data:', responseData); // Añadir depuración
            return responseData;
        } else {
            const errorText = await response.text();
            console.error('Error en la solicitud:', errorText); // Añadir depuración
...
async function showFlowList() {
    try {
        ...
        console.log(flows);  // Verificar que los datos se obtienen correctamente
