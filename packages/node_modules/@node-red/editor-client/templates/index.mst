...
// Métodos para conseguir la autenticación del usuario y poder hacer peticiones al Backend
...
async function getRol(token) {
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/v1/user/rol/${decodedToken.id}`, {
            headers: {
...
// Obtener el JWT del endpoint que busca el valor de la variable de entorno asignada al lanzarse la instancia
...
// Métodos para salir de Node-RED

// Función para redirigir al sistema
...
// Método para detener el servicio de Node-RED
async function stopNodeRedService() {
    try {
        const jwtToken = await getJWTToken();
        const response = await fetch('http://localhost:3000/api/v1/node/stop-nodered', {
            method: 'POST',
            headers: {
...
// Métodos relacionados con el botón Clases

// Método para obtener las clases creadas por el profesor
async function getCreatedClasses(token) {
    try {
        const decodedToken = parseJwt(token);
        const response = await fetch(`http://localhost:3000/api/v1/user/createdclasses/${decodedToken.id}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
// Método para obtener las clases a las que se ha unido el alumno
async function getJoinedClasses(token) {
    try {
        const decodedToken = parseJwt(token);
        const response = await fetch(`http://localhost:3000/api/v1/user/joinedclasses/${decodedToken.id}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
// Método para obtener los flujos de una clase
async function getFlowsForClass(classId, token) {
    const response = await fetch(`http://localhost:3000/api/v1/class/${classId}/flows`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    });
...
// Método para abrir la ventana emergente con la lista de flujos de una clase
async function openFlowsWindow(flows) {
    ...
    for (const flow of flows) {
        try {
            ...
            const userResponse = await fetch(`http://localhost:3000/api/v1/flow/user/${flow._id}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
            ...
            flowItem.onclick = async function() {
                try {
                    const response = await fetch(`http://localhost:3000/api/v1/flow/${flow._id}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
...
// Método para abrir una venta emergente con la lista de clases del usuario
...
// Métodos para todo lo relacionado con el botón Cargar Flujos

// Método para mostrar una ventana emergente con la lista de flujos de la base de datos
async function showFlowListLoad() {
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/v1/user/flows/${decodedToken.id}`, {
            headers: {
                'Authorization': `Bearer ${jwtToken}`
            }
        });
...
// Méotodo para avisar al usuario de que si sale de la página puede perder su trabajo
...
// Aquí se define que hace cada uno de los botones del Navbar
...
// Aquí van todos los métodos relacionados con el botón Guardar Flujos


// Función para transformar un nodo antes de guardarlo, para que luego cuando se cargue en Node-RED se haga correctamente
// No guardamos el nodo tab ya que es simplemente un nodo que identifica la pestaña en la que está ahora mismo dentro de Node-RED
...
// Método para obtener los flujos con sus nodos que están ahora mismo cargados en Node-RED
...
//  Método para guardar un Flujo de trabajo en la base de datos
...
// Método para abrir la ventana emergente y mostrar la lista de flujos para guardar
async function showFlowList() {
    try {
        ...
        async function updateFlow(flowId, flow) {
            ...
            const jwtToken = await getJWTToken();
            try {
                const response = await fetch(`http://localhost:3000/api/v1/flow/${flowId}`, {
